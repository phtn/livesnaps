...
import { type AccountMemberRole, hasAccountMemberRole } from '../../src/lib/accounts/members'
...
export const isDraftSnap = (snap: Doc<'snaps'>) => snap.location_session?.status === 'active'

export const isApplicantReadableSnap = (snap: Doc<'snaps'>) =>
  isDraftSnap(snap) || snap.location_session?.status === 'completed'

/** A submitter can continue their own open capture. */
...
/** A submitter can review their own open or successfully completed capture. */
export async function requireOwnReadableSnap(ctx: Ctx, snap: Doc<'snaps'>) {
  const identity = await ctx.auth.getUserIdentity()
  if (
    !identity ||
    snap.metadata.applicant_token_identifier !== identity.tokenIdentifier ||
    !isApplicantReadableSnap(snap)
  ) {
    throw new ConvexError('Unauthorized.')
  }
  if (!snap.accountId) throw new ConvexError('Unauthorized.')
