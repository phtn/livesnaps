...
vi.mock('../src/lib/convex-client', () => ({
  convexClient: { onUpdate: transport.onUpdate, query: transport.query },
  getConvexAuthState: transport.getAuth,
  subscribeToConvexAuthState: transport.subscribeAuth
}))
...
test('the actual expander stays quiet while closed and shares settings across 50 capture opens', async () => {
  ...
  const props = {
    ...
    endCompletedSession: vi.fn(),
    getCurrentIdToken: vi.fn(),
    getCurrentLocation: vi.fn(),
    isDebugMode: true,
    isLocationPrecise: true,
    sessionUploadId: 'capture',
    locationAccuracyMeters: null
  }
  for (let tick = 0; tick < 50; tick++)
    await act(async () => root?.render(Expander, { ...props, isExpanded: false, locationAccuracyMeters: tick }))
...
test('the actual expander keeps one proof subscription through camera and vision updates', async () => {
  ...
  for (let i = 0; i < 50; i++) {
    await act(async () =>
      transport.subscriptions[0].emit({
