...
/**
 * Snaps arrive at field-capture pace, so 30s keeps the table usefully fresh
...
const readErrorMessage = async (response: Response) => {
  ...
  return 'Unable to load snaps.'
}

/**
 * Reads the admin snap list through the Worker rather than Convex directly.
 *
...
export function useAdminSnaps(limit?: number): AdminSnapsResult {
  const [snaps, setSnaps] = useState<AdminSnapListItem[] | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    const url = limit ? `/api/admin/snaps?limit=${limit}` : '/api/admin/snaps'
    ...
    const load = async () => {
      ...
      try {
        ...
        const payload = (await response.json()) as AdminSnapListItem[]
  ...
  return { error, isLoading, lastUpdatedAt, refresh, snaps }
}
