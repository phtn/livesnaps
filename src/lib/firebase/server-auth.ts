...
import { getFirebaseAdminAuth } from '../firebase-admin/admin'
import { firebaseSessionCookieName } from '../firebase-admin/session'
...
export const getVerifiedGodSession = cache(async (): Promise<VerifiedSession | null> => {
  const session = await getVerifiedSession()
