...
const ADMIN_VERIFIED_PHOTO_PATH = '/api/admin/verification-entries/verified-photo'
...
export default {
  async fetch(request: Request, env: WorkerEnvironment, context: WorkerExecutionContext): Promise<Response> {
    ...
    if (pathname === ADMIN_VERIFIED_PHOTO_PATH) {
      return handleAdminVerifiedPhoto(request, { convexUrl })
    }
