import { fetchReportSettings } from '@/lib/snaps/report-settings-client'
...
export async function downloadSnapFullReport(snap: Doc<'snaps'>) {
  const report = createSnapFullReportDocument(snap, new Date(), (await fetchReportSettings()).excludedFields)
