...
function resolveEngine(choice: EngineChoice): EngineResolution {
  if (choice === "lexical") {
    return {
      engine: "lexical",
      typescript: null,
  ...
  if (choice === "typescript") {
    throw new UsageError(
      "The TypeScript engine was requested, but the analyzer installation does not provide the typescript package.",
    );
  }
...
export function analyzeSource(
  ...
  const typescript =
    options.engine === "lexical" ? null : (options.typescript ?? tryLoadAnalyzerTypeScript());
...
function tryLoadAnalyzerTypeScript(): TypeScriptApi | null {
  try {
    const analyzerRoot = path.resolve(__dirname, "..");
    const analyzerRequire = createRequire(path.join(analyzerRoot, "package.json"));
    ...
    return analyzerRequire(entryPoint) as TypeScriptApi;
  } catch {
    return null;
  }
}

function readUtf8FileBounded(filePath: string): string {
